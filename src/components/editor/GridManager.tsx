...
export const GridManager = ({ open, onClose, config, onChange }: GridManagerProps) => {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
...
const ControlRow = ({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) => (
...
const PresetIcon = ({ rows, cols, active }: { rows: number; cols: number; active: boolean }) => (
  <div
